...
```bash
cd traineon_api/apps/api && npm run build
```
Expected: no errors.
...
git add \
  ...
  traineon_api/apps/api/src/modules/sessions/scoring/scoring.processor.ts \
  traineon_api/apps/api/src/modules/realtime/chat.gateway.ts

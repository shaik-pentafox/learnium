...
export class HealthController {
  ...
  async readiness(): Promise<ReadyResponse> {
    ...
    const probes: Record<string, ProbeResult> = {
      ...
      litellm: results[3]?.status === 'fulfilled' ? results[3].value : { status: 'fail', error: String((results[3] as PromiseRejectedResult).reason) },
    };
  ...
  private async probeLiteLLM(): Promise<ProbeResult> {
    const start = Date.now();
    try {
      const url = this.config.get('LITELLM_BASE_URL', { infer: true });
